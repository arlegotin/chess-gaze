...
- [ ] **Step 1: Verify and fetch the pinned primary-source oracle outside the sandbox**
...
- [ ] **Step 2: Resolve code and asset licensing**
...
- [ ] **Step 3: Verify the six landmark semantics on approved frames**
...
- [ ] **Step 4: Decide H1 before production edits**
...
- [ ] **Step 1: Write RED equation-oracle tests**
...
- [ ] **Step 3: Implement the independent equation boundary**
...
- [ ] **Step 4: Write RED output-sign and inverse-rotation tests**

Cover identity centre `(0,0,1)`, positive camera x -> positive repository yaw,
positive camera y -> positive pitch, a known yaw rotation, the pinned oracle
result, and horizontal-flip equivariance. Legacy profile tests must keep the
existing drawing-derived yaw behavior.

- [ ] **Step 5: Implement row-aligned camera-space conversion**

```python
camera_vector = camera_from_normalized_rotation @ np.asarray(
...
camera_pitch = math.asin(float(camera_vector[1]))
camera_yaw = math.atan2(float(camera_vector[0]), float(camera_vector[2]))
```
...
- [ ] **Step 6: Persist profile, asset, and reproducibility evidence**
...
- [ ] **Step 8: Run the guarded native fixed-frame oracle**
...
- [ ] **Step 1: Run reference and official profiles over every approved clip**
...
- [ ] **Step 2: Build three provenance-safe reports**
...
- [ ] **Step 3: Apply the H1 retention gate**
...
- [ ] **Step 4: If kept, make the corrected profile default and write ADR-0007**
...
git add src/chess_gaze/unigaze_preprocessing.py \
  ...
  src/chess_gaze/model_registry.json \
  tests/chess_gaze/test_unigaze_preprocessing.py \
  tests/chess_gaze/test_unigaze_preprocessing_real_video.py \
  tests/chess_gaze/test_gaze_observation.py \
  tests/chess_gaze/test_frame_observation.py \
